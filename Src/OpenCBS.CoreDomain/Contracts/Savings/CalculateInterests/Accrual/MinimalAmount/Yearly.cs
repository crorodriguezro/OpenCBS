...
{
    public class Yearly : ICalculateInterests
    {
        private SavingBookContract _saving;
        private User _user;
        private int _weekEndDay2;

        public Yearly(SavingBookContract pSaving, User pUser, int pWeekEndDay2)
        {
            _saving = pSaving;
            _user = pUser;
        ...
        public List<SavingInterestsAccrualEvent> CalculateInterest(DateTime pClosureDate)
        {
            ...
            while (DateCalculationStrategy.DateCalculationYearly(lastClosureDate, pClosureDate))
            {
                DateTime cDate = new DateTime(lastClosureDate.Year, lastClosureDate.Month, lastClosureDate.Day, 
                    DateTime.Now.Hour, DateTime.Now.Minute, DateTime.Now.Second);
                DateTime accrualDate = DateCalculationStrategy.GetNextMaturity(cDate,new InstallmentType("Yearly",0,12), _weekEndDay2);
